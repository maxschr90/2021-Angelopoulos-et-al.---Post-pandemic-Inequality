%% Solve Problem with Policy
%% This function solves the households problem for a given policy
function [b, Value_Function, h_choice, i_choice, k_choice, c_choice, cost] = SolveStationaryProblem_Agg_pol(parms, r, wages, Transition, grid_parms, cap, coverage, rr)
    ...
    %% Begin VFI Loop
        ...
        vficrit =1;
            ...
            while vficrit > Convergence_Criterion
                ...
                parfor i = 1:indexsize
                        [BC(i,:),h_choice(i,:),i_choice(i,:),  k_choice(i,:), c_choice(i,:), income(i,:), util(i,:), TV(i,:), cost(i,:)] = VFI_Interpolation_Grid_pol( wages, r, b_lim, h(hx(i)), kap(kapx(i)),i_policy(i,:), k_policy(i,:), B_h, veta_h, theta_h, mu, phi, beta, Transition, b, orderpoly, cap, 0.01, coverage, rr);
                end
...
%% reshape everything
    ...
    cost = reshape(cost,size(cost,1),[],4);    
